...
class Fuel_blog extends Fuel_advanced_module {
	...
	function get_next_post($current_post, $return_method = NULL)
	{
		...
		$posts = $this->get_posts(array('post_date >=' => $current_post->post_date, 'fuel_blog_posts.id !=' => $current_post->id), 'post_date asc, id asc', 1, NULL, $return_method);
		if (!empty($posts))
		{
			return $posts[0];
		}
	...
	{
		$posts = $this->get_posts(array('post_date <=' => $current_post->post_date, 'fuel_blog_posts.id !=' => $current_post->id), 'post_date desc, id desc', 1, NULL, $return_method);
		if (!empty($posts))
		{
			return $posts[0];
		}
